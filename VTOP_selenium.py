...
ext = webdriver.ChromeOptions()  #initializing web-driver
ext.add_argument("--headless")
ext.add_argument("--disable-gpu")
...
def until_sign_in(reg_no,vtop_password):
    global driver,wait,wait1,wait2
...
def get_data_from_timetable(sem_code):
    data = dict()
...
def get_data_from_course_page():
    ...
    while True:
        try:
            ...
            break
        try:
            links = driver.find_elements(By.XPATH, '//table//tr[td="'+str(counter)+'"]//a')     #going through each row pf table
            ...
            if document_name not in res.keys() and len(links) != 0:
                res[document_name] = []
            for i in links:
                if "https://" not in i.get_attribute("href"):
                    res[document_name].append(i.get_attribute("href").split("('")[-1][:-2])  # extracting links
                else:
                    res[document_name].append(i.get_attribute("href"))
            counter += 1
        except:
            counter += 1
            continue
...
def get_complete_data(reg_no,password,sem_code):
    until_sign_in(reg_no,password)
    data = get_data_from_timetable(sem_code)
    final_data = list()
    for i in data.keys():
        check = True
        while True:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    final_data.append([i.split("-")[1].strip(),get_data_from_course_page()])
